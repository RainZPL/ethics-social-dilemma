...
class Principle:
    ...
    def __init__(self, preference=None, happiness=None, contribution=None, envy=None,luck=None, autonomy=None, harm=None,
    ...
    def Users(self):
        user = []
        # users = [self.preference,self.happiness,]
        if self.preference is None:
            for index in range(self.users):
                user.append({'happiness': self.happiness[index],
                             'contribution': self.contribution[index],
                             'envy': self.envy[index],
                             'luck': self.luck[index], 'autonomy': self.autonomy[index], 'harm': self.harm[index],
                             'opportunity': self.opportunity[index]})
        else:
            for index in range(self.users):
                user.append({'preference': self.preference[index], 'happiness': self.happiness[index],
                             'contribution': self.contribution[index],
                             'envy': self.envy[index],
                             'luck': self.luck[index], 'autonomy': self.autonomy[index], 'harm': self.harm[index],
                             'opportunity': self.opportunity[index]})
    ...
    def EnvyFreeness(self, users):
        ...
        for key,val in total_envy.items():
            mean_envy[key] = val/self.users
            tmp.append(val)

        if tmp.count(min(mean_envy.values())) > 1:
            return 'This Principle CAN NOT give the BEST output. Please try another.'
        else:
            return min(mean_envy, key=mean_envy.get), min(mean_envy.values())
    ...
    def DesertBased_Proportionalism(self, users):
        ...
        for key, val in diff_user.items():
            ...
            tmp.append(max(val))
        # print(tmp)
        if tmp.count(max(maxvalue.values())) > 1:
            return 'This Principle CAN NOT give the BEST output. Please try another.'
        else:
            return max(maxvalue, key=maxvalue.get), max(maxvalue.values())
    ...
    def Libertarian_Proportionalism(self, users):
        ...
        for val in max_contribution.values():
            tmp.append(val)
        if tmp.count(max(max_contribution.values())) > 1:
            return 'This Principle CAN NOT give the BEST output. Please try another.'
        else:
            return max(max_contribution, key=max_contribution.get), max(max_contribution.values())
    ...
    def Egalitarianism(self, diction, principle):
        ...
        if tmp.count(min(std.values())) > 1:
            new_distri = {}
            tmp2 = []
            if principle == 1:
                for key, value in diction.items():
                    if np.std(value) == min(std.values()):
                        new_distri[key] = max(value)
                        tmp2.append(max(value))
                if tmp2.count(max(new_distri.values())) > 1:
                    return 'This Principle CAN NOT give the BEST output. Please try another.'
                else:

                    return max(new_distri, key=new_distri.get), min(std.values()), max(new_distri.values())
            elif principle == 2:
                for key, value in diction.items():
                    if np.std(value) == min(std.values()):
                        new_distri[key] = min(value)
                        tmp2.append(min(value))
                if tmp2.count(min(new_distri.values())) > 1:
                    return 'This Principle CAN NOT give the BEST output. Please try another.'
                else:
                    return min(new_distri, key=new_distri.get), min(std.values()), min(new_distri.values())
        else:
            return min(std, key=std.get), min(std.values()), 'nonexistent'
