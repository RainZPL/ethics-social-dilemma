        ...
        if not request.form['allocate_num'].isdigit() or not request.form['resource_num'].isdigit():
            ...
            for char in request.form['resource']:
                if char in string.punctuation:
                    ...
                    return render_template('allocating.html', error=error_r,  allocate_num=allocate_num,
    ...
    if request.method == 'GET':
        ...
        if not request.form['user_num'].isdigit():
            ...
            if '/' in preference:
                ...
                return render_template('preference.html', error=error_p, user_num=user_num, actions=actions)
            else:
                preference = preference.split(',')
                if len(preference) < 2:
                    error_n = 'Please input enough options.'
                    return render_template('preference.html', error=error_n, user_num=user_num, actions=actions)
                if len(up_actions) != len(set(up_actions)):
                    error_l = 'Some Options are the same. Please input enough options.'
                    return render_template('preference.html', error=error_l, user_num=user_num, actions=actions)
...
def deciding_factors(preference, fix_prefer, num_user):
    ...
    if request.method == 'GET':
        ...
        return redirect(url_for('deciding_output', preference=preference, num_user=num_user-1, luck=luck,
                                happiness=happiness, envy=envy, contribution=contribution, autonomy=autonomy, harm=harm,
                                opportunity=opportunity))


@app.route('/deciding/<preference>/<int:num_user>/<luck>/<happiness>/<envy>/<contribution>/<autonomy>/<harm>/<opportunity>', methods=['GET', 'POST'])
def deciding_output(preference, num_user, luck, happiness, envy, contribution, autonomy, harm, opportunity):
    # preference = eval(preference)
    # preference = ', '.join(preference)
    ...
    # print(num_user)
    output = principle.Principle(preference, happiness, contribution, envy, luck, autonomy, harm, opportunity, None, 0,
                                 num_user)
    users = output.Users()
    ...
    if request.method == 'GET':
        return render_template('deciding_output.html', preference=preference, num_user=num_user, luck=luck,
                               happiness=happiness, envy=envy, contribution=contribution, autonomy=autonomy, harm=harm,
                               opportunity=opportunity, utilitarianism=utilitarianism, envyfreeness=envyfreeness,
...
@app.route('/allocating/<resource>/<int:resource_num>/<int:num_user>', methods=['GET', 'POST'])
def allocating_factors(resource, resource_num, num_user):
...
@app.route('/deciding/<resource>/<int:resource_num>/<int:num_user>/<luck>/<happiness>/<envy>/<contribution>/<autonomy>'
           '/<harm>/'
           '<opportunity>', methods=['GET', 'POST'])
def allocating_output(resource, resource_num, num_user, luck, happiness, envy, contribution, autonomy, harm, opportunity):
    luck = eval(luck)
    happiness = eval(happiness)
    envy = eval(envy)
    contribution = eval(contribution)
    ...
    harm = eval(harm)
    opportunity = eval(opportunity)
    output = principle.Principle(None, happiness, contribution, envy, luck, autonomy, harm, opportunity, resource,
                                 resource_num, num_user)
    users = output.Users()
